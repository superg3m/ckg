...
void ckg_cstr_insert(char* str, size_t string_buffer_capacity, const char* to_insert, const u32 index) {
	ckg_assert(str);
	ckg_assert(to_insert);

	const u32 string_buffer_length = ckg_cstr_length(str);
...
void ckg_cstr_insert_char(char* str, size_t string_buffer_capacity, const char to_insert, const u32 index) {
	ckg_assert(str);
	ckg_assert(to_insert);

	u32 string_buffer_length = ckg_cstr_length(str);
	u32 source_length = 1;
...
void ckg_cstr_append(char* str, size_t string_buffer_capacity, const char* to_append) {
	u32 string_buffer_length = ckg_cstr_length(str);
	ckg_cstr_insert(str, string_buffer_capacity, to_append, string_buffer_length);
}

void ckg_cstr_append_char(char* str, size_t string_buffer_capacity, const char to_append) {
	u32 string_buffer_length = ckg_cstr_length(str);
	ckg_cstr_insert_char(str, string_buffer_capacity, to_append, string_buffer_length);
}

void ckg_cstr_clear(char* str) {
	ckg_assert(str);

	size_t string_buffer_length = ckg_cstr_length(str); 
	ckg_memory_zero(str, string_buffer_length);
}

void ckg_cstr_copy(char* str, size_t string_buffer_capacity, const char* to_copy) {
	ckg_assert(to_copy);
	ckg_assert(str);
	...
	ckg_memory_copy(to_copy, str, source_length + 1, string_buffer_capacity);
}
...
char** ckg_cstr_split(const char* str, const char* delimitor) {
	...
	for (int i = 0; i < string_buffer_length; i++) {
		ckg_cstr_append_char(temp_buffer, string_buffer_length + 1, str[i]);
...
Boolean ckg_cstr_starts_with(const char* str, const char* starts_with) {
	...
	if (string_buffer_length == 0 && starts_with_length == 0) {
		return TRUE;
	} else if (str[0] != starts_with[0] || string_buffer_length < starts_with_length) {
		return FALSE;
	}
...
Boolean ckg_cstr_ends_with(const char* str, const char* ends_with) {
	...
	Boolean starts_with_substring = FALSE;
	char* temp_string = ckg_substring(str, start_index, string_buffer_length - 1);
...
char* ckg_cstr_reverse(const char* str) {
	...
	for (int i = string_buffer_length - 1; i >= 0; i--) {
		ckg_cstr_append_char(ret_reversed_string, string_buffer_guarenteed_capacity, str[i]);
	}
